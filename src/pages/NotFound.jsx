...
const NotFound = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      setRedirectTo('/dashboard'); // If logged in, suggest going to the dashboard
    } else {
      setRedirectTo('/'); // If not logged in, suggest going to the home page
    }
  }, []);

  const handleRedirect = () => {
    navigate(redirectTo); // Navigate based on login status
  };
